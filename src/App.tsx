...
const livingTraditionCategories = ['Folk Culture', 'Community Practice', 'Sacred Tradition', 'Festival']
...
function DiscoveryTabs({ active }: { active: string }) {
  const tabs = [['all', 'All Heritage', '/heritage'], ['hidden', 'Hidden Gems', '/hidden-heritage'], ['living', 'Living Traditions', '/heritage?mode=living'], ['food', 'Local Food', '/heritage?mode=food'], ['artisans', 'Artisans', '/artisans'], ['stories', 'Stories', '/stories']]
  return <nav className="discovery-tabs" aria-label="Heritage discovery modes">{tabs.map(([key, label, to]) => <Link key={key} className={active === key ? 'active' : ''} to={to}>{label}</Link>)}</nav>
}
...
function HomePage() {
  ...
  return <>
    ...
    <section className="feature-section"><div className="container"><SectionHeading eyebrow="A different way to travel" title="Follow the living threads" body="Move from a place to the people and practices around it." /><div className="feature-grid">{[['01', 'Living traditions', 'Discover traditions that continue today.', 'heritage'], ['02', 'Local craft', 'Meet artisans and understand the work behind the object.', 'artisans'], ['03', 'Forgotten places', 'Find culturally meaningful places outside the standard itinerary.', 'hidden-heritage'], ['04', 'Regional flavours', 'Taste food traditions connected to local history.', 'stories']].map(([number, title, body, route]) => <Link to={`/${route}`} className="feature-card" key={title}><span>{number}</span><h3>{title}</h3><p>{body}</p><ArrowDownRight size={18} /></Link>)}</div></div></section>
    <section className="container content-section hidden-home-section"><SectionHeading eyebrow="Less-known in our prototype dataset" title="Hidden Heritage" body="Look for places and practices that sit outside a standard itinerary, with context that keeps the claim grounded." action={<ArrowLink to="/hidden-heritage">Explore Hidden Heritage</ArrowLink>} /><div className="heritage-grid">{hiddenHeritage.slice(0, 3).map((item) => <HeritageCard key={item.id} item={item} showHiddenNote favorite={{ active: has('heritage', item.id), onToggle: toggle }} />)}</div></section>
...
function HeritagePage() {
  ...
  const fallback = useMemo(() => heritage.filter((item) => (!search || JSON.stringify(item).toLowerCase().includes(search.toLowerCase())) && (!region || item.regionId.endsWith(region)) && (!category || item.category === category) && (mode !== 'living' || livingTraditionCategories.includes(item.category)) && (mode !== 'food' || item.category === 'Food')), [search, region, category, mode])
...
function HiddenHeritagePage() {
  ...
  const fallback = hiddenHeritage.filter((item) => (!search || JSON.stringify(item).toLowerCase().includes(search.toLowerCase())) && (!region || item.regionId.endsWith(region)) && (!category || item.category === category))
  ...
  return <div className="container page-container hidden-page"><PageHero eyebrow="Hidden Heritage" title="Take the quieter path." body="A focused mode for places and practices marked as less-known in our prototype dataset. It is an invitation to look closer, not a claim of objective discovery." /><DiscoveryTabs active="hidden" /><div className="hidden-intro"><Sparkles size={22} /><div><strong>Less-known in our prototype dataset</strong><p>Why explore this? These entries widen the route beyond a headline attraction while keeping cultural-source verification visible.</p></div></div><div className="hidden-filters"><label>Search<input value={search} onChange={(event) => update('search', event.target.value)} placeholder="Try water, craft or memory" /></label><label>Region<select value={region} onChange={(event) => update('region', event.target.value)}><option value="">All regions</option>{regions.map((item) => <option key={item.id} value={item.slug}>{item.name}</option>)}</select></label><label>Category<select value={category} onChange={(event) => update('category', event.target.value)}><option value="">All categories</option>{categories.map((item) => <option key={item}>{item}</option>)}</select></label></div><DataBanner error={error} />{loading && <LoadingState label="Finding the quieter entries…" />}{!loading && !data.length ? <EmptyState title="No hidden entries match" body="Try a different region or clear the category filter." /> : <div className="heritage-grid">{data.map((item) => <HeritageCard key={item.id} item={item} showHiddenNote />)}</div>}</div>
}
...
function MapPage() {
  ...
  const fallback = source.filter((item) => item.regionId.endsWith(region) && (!category || item.category === category))
...
function ArtisanDetailPage() {
  ...
  return <div className="detail-page artisan-detail"><div className="container"><Link className="back-link" to="/artisans"><ArrowLeft size={15} /> All artisans</Link><DataBanner error={error} /><section className="artisan-profile-hero"><ImageFrame src={item.profileImage} alt={`Portrait of ${item.name}`} className="profile-image" /><div><span className="card-kicker">{item.craft}</span><h1>{item.name}</h1><p className="detail-location"><MapPin size={16} /> {item.location} · {item.yearsOfExperience} years of practice</p><p className="profile-intro">{item.biography}</p><div className="detail-actions"><FavoriteButton favorite={favorite} active={has('artisan', item.id)} onToggle={toggle} /><button className="button primary" onClick={() => setModal('connect')}>Connect <ArrowRight size={16} /></button></div></div></section><div className="artisan-action-row"><button className="button secondary" onClick={() => setModal('experience')}>Request Cultural Experience <ArrowRight size={16} /></button><button className="text-button" onClick={() => setModal('story')}>View Craft Story <ArrowRight size={16} /></button></div><div className="detail-grid"><article className="editorial-copy"><Eyebrow>The craft story</Eyebrow><h2>“{item.craftStory}”</h2><p>{item.biography}</p><h3>Skills</h3><div className="tag-row">{(item.skills ?? item.specialties).map((skill) => <Tag key={skill}>{skill}</Tag>)}</div><h3>Workshop availability</h3><p>{item.workshopAvailability ?? 'Prototype availability on request through a verified local introduction.'}</p><div className="living-card"><span>Connect respectfully</span><p>{item.contactMethod}. This prototype does not process payments or publish private contact details.</p></div></article><aside className="detail-aside"><div className="gallery-grid">{item.gallery.map((src, index) => <ImageFrame key={src} src={src} alt={`${item.craft} detail ${index + 1}`} className={index === 0 ? 'gallery-wide' : ''} />)}</div></aside></div><section className="artisan-why"><Eyebrow>Why this matters</Eyebrow><h2>Jeevant Virasat connects cultural discovery with the people who keep traditions alive.</h2><p>Every connection should begin with consent, context and a clear understanding that this is a prototype workflow.</p></section>{related.length > 0 && <section className="related-section"><SectionHeading eyebrow="Follow the practice" title="Related heritage" /><div className="heritage-grid">{related.map((place) => <HeritageCard key={place.id} item={place} />)}</div></section>}</div>{modal && <PrototypeModal title={modal === 'connect' ? `Connect with ${item.name}` : modal === 'experience' ? 'Request Cultural Experience' : `${item.name}'s craft story`} onClose={() => setModal(null)}>{modal === 'story' ? <p>{item.craftStory}</p> : <><p>Prototype interaction — community connection workflow.</p><p>{modal === 'experience' ? `Your request would be prepared for ${item.name}, subject to consent, availability and local coordination.` : `A production version would route an introduction to ${item.name} through a verified local partner.`}</p><JoinNotice text="No booking, payment or WhatsApp message has been sent." /></>}<div className="modal-actions"><button className="button primary" onClick={() => setModal(null)}>Close</button></div></PrototypeModal>}</div>
}
